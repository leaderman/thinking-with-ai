...
X 是 SPA，推文数据不在 HTML 里，而是通过 GraphQL 接口异步加载。整体流程如下：

1. **建立 CDP 连接** — 通过 Chrome 的 HTTP 接口开一个新 Tab，用 WebSocket 连接 Chrome DevTools Protocol
2. **挂载拦截监听器** — 在页面导航之前就挂上网络事件监听，确保不遗漏任何请求
3. **打开目标页面** — 页面加载完成后，X 会自动发出第一次 `UserTweets` 请求，无需滚动
4. **持续滚动** — 开始滚动页面，但不是每次滚动都会触发接口，X 的懒加载策略是判断当前内容快显示完时才发下一批请求
5. **拦截到一次处理一次** — 每当捕获到一次 `UserTweets` 响应就立即解析并追加到结果集，直到达到目标数量或连续多次滚动没有新数据为止
